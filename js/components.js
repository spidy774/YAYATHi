...
function renderNav(currentPage) {
  const links = NAV_ITEMS.map(
    (item) =>
      `<li><a href="${item.href}"${item.href === currentPage ? ' aria-current="page"' : ""}>${item.label}</a></li>`
  ).join("");
...
function memberCardHTML(member) {
  const socials = Object.entries(member.socials || {})
    ...
    .map(([key, url]) => {
      ...
      return `<a href="${url}" target="_blank" rel="noopener noreferrer" aria-label="${label}">${label[0]}</a>`;
    })
    .join("");
